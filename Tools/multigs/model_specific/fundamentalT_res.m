...
% Edit by Alex Xun Xu: Changed line 31 dist = zeros(1,n); to dist = inf*ones(1,n);
%   to avoid all-zero assignment to distance

function [dist, P] = fundamentalT_res(F,X)
...
if iscell(F)  % We have several solutions each of which must be tested
    ...
    % control if point correspondences are less than 3
    if size(x1, 2) < 3
