...
function [ par res inx tim ] = randomSampling(lim,data,M,model_type)
% input: 
...
for m=1:M    
    ...
    while (isdegen==1)&&(degencnt<=degenmax)
        ...
        if isempty(strfind(model_type,'fundamental')) && isempty(strfind(model_type,'fundamentala')) && isempty(strfind(model_type,'fundamentalt'))
