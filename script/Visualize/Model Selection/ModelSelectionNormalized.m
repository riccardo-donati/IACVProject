...
% In this version we normalize the process of model fitting in the second
% phase
%%
        ...
        for model=1:size(models,2)
            ...
            for m=1:nMotions
                try
                    ...
                    %Models{m} = feval(fitModel, [matchedPoints1{m}(:,1:sizeMin); matchedPoints2{m}(:,1:sizeMin)]); %deterministic
                catch 
                    Models{m} = NaN;
                end
            end
            ...
            for m=1:nMotions
                if isnan(Models{m})
                    ...
                    l = load("BestLambdasNormalized.mat").lambdas;
                    lambda1T = l(1);
                    lambda2T = l(2);
                    lambda3T =  l(3);
...
for i=1:length(mod_sel)
    ...
    for seq=1:length(SeqList)
        ...
        for fr=1:Data.nFrames-FrameGap
            ...
            m = cell2mat(squeeze(results(fr,:,:))); % se 0 corrispondenze, prende il primo modello dall'array -> fundamental
            [val,idx] = min(m);
            curr_best=[];
    ...
    for seq = 1:length(SeqList)
        ...
        for f_i = 1:Data.nFrames-FrameGap
            ...
            for m = 1:nMotions
                ...
                % if model_type is None, use fundamental
                if curr_best_models(f_i,m) == "None"
            ...
            for m=1:nMotions
                for n=1:nMotions
                    if curr_best_models(f_i,n) == "None"
                        model_type = "fundamental";
                    else
