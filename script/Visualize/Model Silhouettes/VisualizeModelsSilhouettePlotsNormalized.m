...
% This is the version in which the fit of the model is normalized in
...
for model=1:length(models)
    ...
    for seq = 1:length(SeqList)
        ...
        for f_i = 1:Data.nFrames-FrameGap
            ...
            for m=1:nMotions
                for n=1:nMotions
                    if model_type ~='homography'
                        if Models{n} == [0 0 0; 0 0 0; 0 0 1]
                            ev = feval(resModel, Models{n},[matchedPoints1{m}; matchedPoints2{m}]);
                            ReswrtMotion{m}{n} =NaN(size(ev));
            ...
            for m=1:nMotions
                ...
                scoresMotion{seq}{f_i}{m} = (b - a) ./ max(a, b);% * (1-ratios(m));
                silhouetteMotion{seq}{f_i}{m} = nanmean(scoresMotion{seq}{f_i}{m});
                silhouettes(m, f_i) = nanmean(scoresMotion{seq}{f_i}{m});
        ...
        n1 = n1 + 7;
        n2 = n2 + 7;
        writetable(T,"ModelSilhouettesNormalized/"+model_type+"/silhouetteScores_"+model_type+".xls",'WriteRowNames',true,'Sheet',1,'Range',cell)
    end
    

    %% plot every frame silhouette of all clips for every model
    
    imageFolder = strcat("ModelSilhouettesNormalized/",models(model));
    for seq=1:length(SeqList)
    ...
    set(gca,'FontSize',24)
    saveas(f, "ModelSilhouettesNormalized/"+models(model)+"AvgSilhouetteScorePerMotion.png");
    ...
    set(gca,'TickLabelInterpreter','none')
    set(gca,'FontSize',24)
    saveas(f, "ModelSilhouettesNormalized/"+models(model)+"AvgSilhouetteScore.png");
