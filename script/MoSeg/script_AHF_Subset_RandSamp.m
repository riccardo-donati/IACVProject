...
for Alpha = Alpha_Range
    ...
    for gamma = gamma_range
        ...
        for s_i = seq_range
            ...
            %%% Save clustering and U for every sequence to later evaluate silhouette coefficients
            %%% it will be called as: silhouette(U_matrices{s_i}, ClusterIdx{s_i})
            observations{s_i} = U;
            [ClusterIdx{s_i},~,wss] = kmeans(U, nMotion, 'replicates',500, 'start', 'cluster', ...
            ...
            %%% Silhouette coefficients for every clustering (every clip)
        ...
        save(result_filepath,'observations', 'ClusterIdx','error','allWSS');
