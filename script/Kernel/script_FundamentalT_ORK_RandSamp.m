...
model_type = lower('fundamentalt');   % Model name
...
for s_i = seq_range
    ...
    for f_i = 1:Data.nFrames-FrameGap
        ...
        %% Compute ORK kernel
        [ foo, resinx] = sort(Res,2); % sort (ascending) Res along dimension 2 e return sorted foo and sorted indexs
        h = round(0.1*size(Res,2)); % threshold -> prende gli i primi 50 modelli F per ogni (coppia di)punto con minor residual 
             % e assegna a quel punto i 50 modelli che più lo fittano 
             % computa la similarity fra punti facendo kij = oi'oj
             % e poi la somma ripetendo per ogni coppia di frame nella
             % sequenza 
